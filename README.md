...
当前版本只做一件事：通过 Rivioo 的导出能力下载 APP 评论，并把原始 CSV/XLSX 文件保存到本地。评论解析、聚类、情绪分析和体征归因放到后续版本。

服务启动时会从 `https://www.rivioo.app/` 自动识别 Rivioo 当前后端接口。如果 Rivioo 更换首页暴露的 API 地址，本地后台重启后会自动使用新地址，也可以在页面右上角点击“重新识别”。
